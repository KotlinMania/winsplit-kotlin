...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`winsplit`](https://crates.io/crates/winsplit).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `winsplit` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

`CommandLineToArgvW`-equivalent argument splitter
...
Original work copyrighted by the upstream `winsplit` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
