...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Winsplit.swiftmodule/`
...
//      `import Winsplit` above would fail at compile time.
//
//   2. The static archive `libWinsplit.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Winsplit" }` and
//      `flattenPackage = "io.github.kotlinmania.winsplit"` configuration in
...
//      via the `WinsplitLibrary` product.
//
...
final class WinsplitExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Winsplit swift module imported cleanly")
    }
}
