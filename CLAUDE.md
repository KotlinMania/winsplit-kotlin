# Claude Code Project Instructions — winsplit-kotlin

## Project Overview

This is **winsplit-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`winsplit`](https://crates.io/crates/winsplit).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/winsplit/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/winsplit>
